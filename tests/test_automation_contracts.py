...
class AutomationContractTests(unittest.TestCase):
    ...
    def test_automation_contract_registry_is_deny_by_default_and_valid(self) -> None:
        ...
        self.assertEqual(
            state["allowed_action_types"],
            [
                "rank_math_homepage_meta_description_update",
                "rank_math_meta_description_update",
            ],
        )
    ...
    def test_rank_math_homepage_meta_description_contract_requires_admin_and_rollback(self) -> None:
        contract = get_automation_contract(self.registry, "rank_math_homepage_meta_description_update")
        self.assertIsNotNone(contract)
        assert contract is not None
        self.assertEqual(contract["contract_id"], "ac-rank-math-homepage-meta-description-update-v1")
        self.assertTrue(contract["requires_admin_confirmation"])
        self.assertTrue(contract["requires_before_state_capture"])
        self.assertTrue(contract["requires_rollback"])
        self.assertIn("recommend_only", contract["allowed_runtime_gates"])
        self.assertIn("rank_math_titles.homepage_description", contract["allowed_target_fields"])
    ...
    def test_homepage_candidate_can_attach_homepage_option_contract(self) -> None:
        contract = get_automation_contract(self.registry, "rank_math_homepage_meta_description_update")
        assert contract is not None
        candidate = {
            "action_type": "rank_math_homepage_meta_description_update",
            "execution_lane": "admin_confirmed_assisted_resolution_only",
            "runtime_gate": "recommend_only",
            "active_seo_owner": "seo-by-rank-math/rank-math.php",
            "bound_domain": "electri-c-ity-studios-24-7.com",
            "target_domain": "electri-c-ity-studios-24-7.com",
            "target_field": "rank_math_titles.homepage_description",
            "approval_state": "admin_confirmation_required",
            "rollback_state": "ready_if_before_state_captured",
        }
        self.assertEqual(candidate_contract_issues(candidate, contract), [])
        enriched = attach_contract(candidate, contract)
        self.assertEqual(enriched["automation_contract_state"], "contract_verified")
        self.assertEqual(enriched["automation_contract_id"], "ac-rank-math-homepage-meta-description-update-v1")
        self.assertEqual(enriched["scope"], "exact_domain_rank_math_homepage_option_only")
