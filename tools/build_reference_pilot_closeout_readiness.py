...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a reference-pilot closeout readiness dossier from an installed bridge runtime export."
    )
    parser.add_argument("--input", required=True, help="JSON export from the installed bridge runtime.")
    parser.add_argument("--output", required=True, help="Target JSON file for the local closeout dossier.")
...
def main() -> int:
    ...
    closeout = build_reference_pilot_closeout_readiness(export_payload)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(closeout, indent=2) + "\n", encoding="utf-8")
